...
export function useCSVImport() {
  ...
  const parseRow = (row: any[], rowIndex: number): ParsedCSVRow => {
    ...
    const parsePPM = (val: any) => val === undefined || val === null || val.toString().trim() === "" ? null : parseIndianNumber(val.toString());
    ...
    return {
      ...
      janPPM: parsePPM(row[13]), febPPM: parsePPM(row[14]),
      marPPM: parsePPM(row[15]), aprPPM: parsePPM(row[16]),
      mayPPM: parsePPM(row[17]), junPPM: parsePPM(row[18]),
      julPPM: parsePPM(row[19]), augPPM: parsePPM(row[20]),
      sepPPM: parsePPM(row[21]), octPPM: parsePPM(row[22]),
      novPPM: parsePPM(row[23]), decPPM: parsePPM(row[24]),
      edlId: row[25]?.toString().trim() || "",
      edlName: row[26]?.toString().trim() || "",
      pdlId: row[27]?.toString().trim() || "",
      pdlName: row[28]?.toString().trim() || "",
      pmId: row[29]?.toString().trim() || "",
      pmName: row[30]?.toString().trim() || "",
      opportunityId: row[31]?.toString().trim() || "",
      opportunityName: row[32]?.toString().trim() || "",
      projectEndDate: row[33]?.toString().trim() || "",
      soId: row[34]?.toString().trim() || "",
      jan: parseNum(row[35]), feb: parseNum(row[36]), mar: parseNum(row[37]),
      apr: parseNum(row[38]), may: parseNum(row[39]), jun: parseNum(row[40]),
      jul: parseNum(row[41]), aug: parseNum(row[42]), sep: parseNum(row[43]),
      oct: parseNum(row[44]), nov: parseNum(row[45]), dec: parseNum(row[46]),
      lastSubmissionTotal: parseNum(row[47]),
      janActual: parseNum(row[48]),
      currentSubmissionTotal: parseNum(row[61]),
      lastVsCurrent: parseNum(row[62]),
      reason: row[63]?.toString().trim() || "",
    };
  };
        ...
        if (!projectMaster) {
          const pmResponse = await fetch(`${API_BASE}/project-masters`, {
            ...
            body: JSON.stringify({
              ...
              febPPM: row.febPPM || undefined, marPPM: row.marPPM || undefined,
              aprPPM: row.aprPPM || undefined, mayPPM: row.mayPPM || undefined,
              junPPM: row.junPPM || undefined, julPPM: row.julPPM || undefined,
              augPPM: row.augPPM || undefined, sepPPM: row.sepPPM || undefined,
              octPPM: row.octPPM || undefined, novPPM: row.novPPM || undefined,
              decPPM: row.decPPM || undefined,
            }),
          });
