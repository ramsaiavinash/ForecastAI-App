...
const calculateEstimatedForecast = (row, projectEndDate) => {
    const result = {};
...
const parseDate = (dateStr) => {
    if (!dateStr || dateStr === "(blank)" || dateStr === "")
...
router.post("/csv", async (req, res) => {
    try {
        ...
        for (const row of rows) {
            ...
            let project;
            try {
                project = await prisma_1.prisma.projectMaster.upsert({
            ...
            for (const [monthStr, data] of Object.entries(monthData)) {
                ...
                try {
                    await prisma_1.prisma.monthlyRevenue.create({
                        data: {
                            batchId: batch.id,
