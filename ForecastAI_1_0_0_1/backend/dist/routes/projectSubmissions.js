...
router.get("/batch/:batchId", async (req, res) => {
    try {
        const submissions = await prisma_1.prisma.projectSubmission.findMany({
...
router.post("/", async (req, res) => {
    try {
        ...
        const batchProjects = await prisma_1.prisma.monthlyRevenue.findMany({
            where: { batchId, projectId: { in: projectIds } },
            select: { projectId: true },
            distinct: ["projectId"],
        });
        const validProjectIds = batchProjects.map(project => project.projectId);
        const submissions = await prisma_1.prisma.$transaction(validProjectIds.map(projectId => prisma_1.prisma.projectSubmission.upsert({
