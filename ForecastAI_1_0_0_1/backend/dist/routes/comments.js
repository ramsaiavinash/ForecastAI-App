...
function normalizeComment(comment) {
    const sentBy = comment.sentBy || (comment.commentedBy === "Practice Lead" ? "PL" :
...
async function getThreadedComments(where) {
    const comments = await prisma_1.prisma.projectComment.findMany({ where, orderBy: { sentAt: "asc" } });
...
router.post("/", async (req, res) => {
    try {
        ...
        const sender = sentBy || (commentedBy === "Practice Lead" ? "PL" :
        ...
        const created = await prisma_1.prisma.projectComment.create({
            data: {
...
router.put("/:id", async (req, res) => {
    try {
        ...
        if (status !== "Resolved")
            return res.status(400).json({ error: "Only resolution is supported" });
        const comment = await prisma_1.prisma.projectComment.findUnique({ where: { id: req.params.id } });
        if (!comment)
            return res.status(404).json({ error: "Comment not found" });
        const root = comment.parentId
            ? await prisma_1.prisma.projectComment.findUnique({ where: { id: comment.parentId } })
            : comment;
        if (!root || root.sentBy !== resolvedBy) {
            return res.status(403).json({ error: "Only the query author can resolve this thread" });
        }
        const updated = await prisma_1.prisma.projectComment.update({ where: { id: root.id }, data: { status: "Resolved" } });
...
router.delete("/:id", async (req, res) => {
    try {
        await prisma_1.prisma.projectComment.deleteMany({ where: { parentId: req.params.id } });
        await prisma_1.prisma.projectComment.delete({ where: { id: req.params.id } });
